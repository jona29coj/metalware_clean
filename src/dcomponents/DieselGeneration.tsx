...
const DieselGeneration = () => {
  ...
  const [dgData, setDgData] = useState<any[] | null>(null);
  ...
  useEffect(() => {
    const fetchDgDetails = async () => {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/dgdc?startDateTime=${startDateTime}&endDateTime=${endDateTime}`);
  ...
  const dg1: DgReading = dgData && dgData[13] ? dgData[13] : { total_kW: null, timestamp: null };
  const dg2: DgReading = dgData && dgData[14] ? dgData[14] : { total_kW: null, timestamp: null };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col items-center text-center p-6 bg-gray-50 rounded-lg shadow-md">
          ...
          <div className="text-gray-700 text-sm space-y-2">
            ...
            <p>
              <strong>Last Updated:</strong> {dg1.timestamp ? moment.tz(dg2.timestamp as string, "YYYY-MM-DD HH:mm:ss", "Asia/Kolkata").format("YYYY-MM-DD HH:mm:ss") : 'N/A'}
            </p>
          </div>
          <button
