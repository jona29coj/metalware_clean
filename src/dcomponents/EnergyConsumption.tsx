...
const EnergyHeatmap = () => {
  ...
  const [viewStartDate, setViewStartDate] = useState<string>(startDate);
  const [viewEndDate, setViewEndDate] = useState<string>(endDate);

  const [consumptionData, setConsumptionData] = useState<ConsumptionDatum[]>([]);
  ...
  const fetchConsumptionData = async (start: string, end: string) => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/ehconsumption`, {
  ...
  const handleDownloadExcel = () => {
    ...
    const formattedData = consumptionData.map((item) => [
      item.day,
      `${item.hour}:00`,
      parseFloat(item.total_consumption as string),
    ]);
  ...
  consumptionData?.forEach(({ day, hour, total_consumption }) => {
    ...
    if (dayIndex !== -1 && hour >= 0 && hour < 24) {
      heatmapData[hour][dayIndex] = parseFloat(total_consumption as string);
    }
  });

  const maxValue = Math.max(...heatmapData.flat().filter((v) => v !== null) as number[]) || 1;
