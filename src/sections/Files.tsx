...
type FileCategory = "Energy Bills" | "Manuals" | "Miscellaneous";

const Files = () => {
  const [selectedCategory, setSelectedCategory] = useState<FileCategory>("Energy Bills");
  const [files, setFiles] = useState<Record<FileCategory, FileEntry[]>>({
    "Energy Bills": [],
    Manuals: [],
    Miscellaneous: [],
  });
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/api/uploadF?category=${selectedCategory}&cacheBuster=${Date.now()}`,
        formData,
      ...
      updateFiles(selectedCategory, {
        ...
        url: `${import.meta.env.VITE_API_BASE_URL}${fileUrl}`,
      });
  ...
  const handleRemoveFile = async (index: number) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/api/delete`, {
  ...
  const updateFiles = (category: FileCategory, newFile: FileEntry | null, removeIndex?: number) => {
    setFiles((prevFiles) => ({
      ...prevFiles,
      [category]: removeIndex !== undefined
        ? prevFiles[category].filter((_, i) => i !== removeIndex)
        : [...prevFiles[category], newFile as FileEntry],
    }));
  };
  ...
  const fetchFiles = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}/api/listF?category=${selectedCategory.toLowerCase()}`
      );

      const formattedFiles = response.data.map((file: any) => ({
        name: file.name,
        date: file.date,
        url: file.url.startsWith("http") ? file.url : `${import.meta.env.VITE_API_BASE_URL}${file.url}`,
      }));
...
const FileTable = ({ files, onDelete, onUpload, category }: FileTableProps) => (
