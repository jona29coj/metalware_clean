...
const EnergyMeter = ({ name, consumption, id }: EnergyMeterProps) => {
...
const MeterInfo = () => {
  ...
  const [energyMeters, setEnergyMeters] = useState<EnergyMeterEntry[]>(
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/econsumption`, {
          params: {
            startDateTime,
            endDateTime
          }
        ...
        const formattedData = response.data.consumptionData.map((entry: any) => ({
        ...
        const totalConsRes = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/mcapcons`, {
