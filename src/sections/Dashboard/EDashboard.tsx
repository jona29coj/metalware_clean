...
const getCurrentRate = (hours: number) => {
...
const Edmc = ({data, period, rate}: {data: any; period: string; rate: string}) => {
...
const DieselGeneration = ({ data }: {data: any}) => {
  ...
  const getStatusColor = (status: string) => (status === 'Running' ? 'text-green-600' : 'text-red-600');
...
const EnergyHeatmap = ({ data }: {data: any}) => {
...
const [consumptionData, setConsumptionData] = useState<any[]>(data || []);
...
useEffect(() => {
  ...
  const fetchConsumptionData = async (start: string, end: string) => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/ehconsumptiontest`, {
  ...
  const daysDiff =
    Math.ceil((new Date(viewEndDate).getTime() - new Date(viewStartDate).getTime()) / 86400000) + 1;
  ...
  const heatmapData: (number | null)[][] = Array(24)
    .fill(0)
    .map(() => Array(daysDiff).fill(null));

  consumptionData?.forEach(({ day, hour, total_consumption }: any) => {
  ...
  const monthSeparators: {position: number; name: string}[] = [];
...
const zoneDetails: Record<number, ZoneDetail> = {
...
const getZoneNameAndCategory = (id: number): { name: string; category?: string | null } => {
if (id === 0) return { name: "TOTAL CONSUMPTION", category: null };
return zoneDetails[id] || { name: "Unknown Zone", category: "N/A" };
};

const HConsumption = ({data}: {data: any}) => {
  const { startDateTime, endDateTime } = useContext(DateContext)!;
  const options = ['kVAh', 'kWh', '₹'];
  const [warning, setWarning] = useState('');
  const [energyData, setEnergyData] = useState<Record<string, any>>(data?.hourlyConsumption || {});
...
const EnergyMeter = ({ name, consumption, id }: {name: string; consumption: number; id: number}) => {
  const navigate = useNavigate();
...
const MeterInfo = ({ data }: {data: any}) => {
  const meters = data?.meterWiseConsumption || [];

  const energyMeters = Array.from({ length: 12 }, (_, i) => {
    const meterId = i + 1;
    const matched = meters.find((m: any) => m.energy_meter_id === meterId);
    return {
      id: meterId,
      name: `Zone ${meterId}`,
...
const PeakDemand = ({ data }: {data: any}) => {
  const { startDateTime, endDateTime } = useContext(DateContext)!;
  const [warning, setWarning] = useState('');
  ...
  const downloadExcel = () => {
    ...
    const formattedData = peakDemandData.map((item: any) => [
  ...
  const categories = peakDemandData.map((d: any) => moment(d.minute).format("HH:mm"));

  const options: ApexOptions = {
    chart: { type: "line", background: "white", toolbar: { show: false } },
    xaxis: {
      categories,
  ...
  const series = [
    {
      name: "Apparent Power",
      data: peakDemandData.map((d: any) => parseFloat(d.total_kVA)),
    },
  ];
...
const ZoneUsage = ({data}: {data: any}) => {
  const meterToZoneMap: Record<number, ZoneDetail> = {
    1: { name: "PLATING", category: "C-49" },
  ...
  useEffect(() => {
    ...
    const updatedZones = zoneData.map((zone) => {
      const matchingEntry = data.meterWiseConsumption.find(
        (entry: any) => entry.energy_meter_id === zone.id
      );
      return {
        ...zone,
  ...
  useEffect(() => {
    ...
    zoneData.forEach((zone) => {
      ...
      const material = new THREE.MeshBasicMaterial({
        color: categoryColors[zone.category as string],
      });

      const cube = new THREE.Mesh(geometry, material);
      const edgesGeometry = new THREE.EdgesGeometry(geometry);
      const edgesMaterial = new THREE.LineBasicMaterial({
        color: highlightColors[zone.category as string],
      });
      ...
      cube.userData = { ...zone, originalColor: categoryColors[zone.category as string] };
      ...
      if (intersects.length > 0) {
        ...
        if (intersected !== currentIntersected) {
          if (currentIntersected) {
            (currentIntersected.material as THREE.MeshBasicMaterial).color.set(currentIntersected.userData.originalColor);
          }

          currentIntersected = intersected;
          (intersected.material as THREE.MeshBasicMaterial).color.set(highlightColors[intersected.userData.category]);
...
const EDashboard = () => {
  ...
  const [dashboardData1, setDashboardData1] = useState<any>({});
  const [dashboardData2, setDashboardData2] = useState<any>({});
  const [totalCost, setTotalCost] = useState(0);
  const [ehconsumption, setEhConsumption] = useState<any[]>([]);
  const [startDate, setStartDate] = useState(() =>
  moment.tz("Asia/Kolkata").subtract(30, "days").format("YYYY-MM-DD")
);
const [endDate, setEndDate] = useState(() =>
  ...
  useEffect(() => {
    const fetchSequentially = async () => {
      try {
        const res1 = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/dashboardpt1test`, {
          params: { startDateTime, endDateTime }
        });
        setDashboardData1(res1.data);

        const res2 = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/dashboardpt2test`, {
          params: { startDateTime, endDateTime }
        });
        setDashboardData2(res2.data);

        const res3 = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/ehconsumptiontest`, {
