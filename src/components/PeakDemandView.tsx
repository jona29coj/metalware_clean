...
const PeakDemandView = () => {
  const [peakDemandData, setPeakDemandData] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchAlertData = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/apdtest`, {
          params: { startDateTime, endDateTime },
        });

        const peakAlerts = response.data.peakDemandAboveThreshold.map((item: any) => ({
          type: 'Peak Demand',
        ...
        const dgAlerts: any[] = [];
        response.data.dgActivations.forEach((event: any) => {
