...
const meterToZoneMap: Record<number, { name: string; category?: string }> = {
...
const MonDailReading = () => {
  ...
  const [data, setData] = useState<ShiftEntry[]>([]);
    ...
    const fetchShiftData = async () => {
      ...
      for (let m = moment(start); m.isSameOrBefore(end); m.add(1, "days")) {
        ...
        const [s1, s2] = await Promise.all([
          axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/meterreading`, {
            params: {
  ...
  const downloadExcel = () => {
    const rows: any[] = [["Date", "Zone", "Shift", "Start Timestamp", "End Timestamp", "Start kVAh", "End kVAh", "Start kWh", "End kWh"]];

    data.forEach((entry) => {
      (["shift1", "shift2"] as const).forEach((shiftKey, shiftIndex) => {
        const shiftLabel = shiftIndex === 0 ? "Shift 1 (8AM–8PM)" : "Shift 2 (8PM–8AM)";
        entry[shiftKey].forEach((zoneData: any) => {
    ...
    <div className="overflow-x-auto">
      ...
      {data.map((entry, idx) => (
        <div key={idx} className="mb-8 border border-gray-300 p-4 rounded-md">
          ...
          {(["shift1", "shift2"] as const).map((shiftKey, shiftIndex) => (
            <div key={shiftKey}>
              ...
              <table className="min-w-full border-collapse border border-gray-300 text-sm mb-4">
                ...
                <tbody>
                  {entry[shiftKey].map((zoneData: any, index: number) => (
