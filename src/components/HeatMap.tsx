...
export default function HeatMap() {
  ...
  const [data, setData] = useState<number[][]>(
    Array(numDays).fill(0).map(() => Array(24).fill(0).map(() => Math.floor(Math.random() * 401)))
  );
  ...
  const getColor = (v: number) => colorValues[Math.min(Math.floor(v / 29), colorValues.length - 1)];
